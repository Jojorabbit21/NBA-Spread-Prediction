...
df = pd.read_csv('data/schedules/2019-2022_clean.csv')
...
df_vis = pd.get_dummies(df['away'], dtype=np.int64)

# Create training model

df_model = df_home.sub(df_vis)
df_model['pt_diff'] = df['pt_diff']
df_train = df_model
...
x = df_train.drop(['pt_diff'], axis = 1)
y = df_train['pt_diff']
...
df_odds = pd.read_csv('data/odds/2019-2022_clean.csv')
df_odds = df_odds[['home', 'away', 'home_total', 'away_total', 'home_ml', 'away_ml']]
...
df_odds['away_ml'] = df_odds['away_ml'].astype(int)
df_odds['away_ml'] = np.where(df_odds['away_ml'] > 0, (df_odds['away_ml'] / 100) + 1,  (100 / abs(df_odds['away_ml']) + 1))
...
df_odds['bet_home']  = np.where(df_odds['home_rat'] > df_odds['away_rat'] , 1, 0)
df_odds['home_win'] = np.where(df_odds['home_total'] > df_odds['away_total'] , 1, 0)
...
df_odds['bet_odds'] = np.where(df_odds['bet_home'] == 1 , df_odds['home_ml'], df_odds['away_ml'])
df_odds['winnings'] = np.where(df_odds['bet_win'] == 1, (df_odds['bet_odds'] * 100) - 100, -100)

print("Bets won: " + str(df_odds['bet_win'].sum()))
print("Bets lost: " + str(df_odds['bet_lose'].sum()))
print("Winnings: " + str(df_odds['winnings'].sum()))

df_odds.to_csv('result.csv')
